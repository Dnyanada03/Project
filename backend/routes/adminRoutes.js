...
// Issue Certificate for an approved application
router.put('/applications/:id/issue-certificate', async (req, res) => {
    try {
        const activity = await Activity.findByIdAndUpdate(
            req.params.id,
        ...
        if (!activity) return res.status(404).json({ message: 'Application not found' });
        res.json({ message: 'Certificate issued successfully', activity });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
});
